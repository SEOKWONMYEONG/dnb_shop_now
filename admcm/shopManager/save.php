...
if($mode == "main_banner_add"){
    ...
    echo $response;
}elseif($mode == "main_banner_update"){
    $code = $_POST['code'];
    $text =$_POST['text'];
    $idx = $_POST['idx'];
    ...
    if ($_FILES["filename"]["name"]!=""){
        ...
        if (move_uploaded_file($_FILES["filename"]["tmp_name"], $targetFile)) {
            $filename = $uploadTime . "_" . $_FILES["filename"]["name"];
            $sql = "UPDATE banner_main SET code='$code', filename='$filename', text='$text' where idx =".$idx;

            mysqli_query($conn,$sql);
            $response = "배너가 수정되었습니다.";
        } else {
            $response =  "배너 수정이 실패하었습니다.";
        }
    }else{
